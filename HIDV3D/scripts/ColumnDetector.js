...
class ColumnDetector {
  ...
  detectBestColumns() {
    const analyses = this.columns.map((col) =>
    ...
    // Try to find 3 numeric columns first
    if (numericCols.length >= 3) {
      ...
      // All categorical - use first 3
      selected.x = this.columns[0];
      selected.y = this.columns[1] || this.columns[0];
      selected.z = this.columns[2] || this.columns[1] || this.columns[0];
    }
